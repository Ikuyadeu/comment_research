...
# Input csv
num <- read.csv("CSVdata2_q/numOfPatchsets_2.csv"
                 ,sep=",", header=TRUE)
...
for(i in index){
  for(j in i){ # first Comment
    for(k in j){ # second Comment
      ...
      jpeg(paste("picture2_q/RQ2_2",name,".jpg",sep=""))
      boxplot(value1, value2, xlab="Reviewer order", ylab="Reliability", main=name, ylim=c(0,1))
      print(name)
    }
  }
}
