...
for(i in index){
  for(j in i){ # first Comment
    ...
    for(k in i){ # second Comment
      ...
      for(l in i){ # second Comment
        value3 = l[[3]]$CurrentPar
        vote = paste("Vote:", j[[1]]$VotingScore[1], ",", k[[2]]$VotingScore[1], ",", l[[3]]$VotingScore[1],  " ")
        boxplot(value1, value2, value3, xlab="Reviewer order", ylab="Reliability", main=paste(vote, "Status:", j[[1]]$Status[1]))
        summary(value1)
        summary(value2)
        summary(value3)
      }
